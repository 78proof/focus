...
export class OutlookService {
  ...
  private static getClientId() {
    const id = localStorage.getItem('OUTLOOK_CLIENT_ID');
  ...
  static async fetchEmails() {
    ...
    if (!token) return [];
    try {
      const response = await fetch("https://graph.microsoft.com/v1.0/me/messages?$top=15&$select=id,subject,from,bodyPreview,receivedDateTime,importance", {
